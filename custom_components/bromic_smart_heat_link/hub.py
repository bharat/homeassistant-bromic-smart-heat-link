...
class BromicHub:
    ...
    @callback
    def _notify_connection_state(self, connected: bool) -> None:  # noqa: FBT001
    ...
    async def async_connect(self) -> None:
        ...
        try:
            ...
            _LOGGER.exception("Failed to connect to %s", self.port)
            self._notify_connection_state(connected=False)
            raise BromicConnectionError from err
    ...
    def _connect(self) -> None:
        ...
        try:
            ...
            message = f"Serial port error: {err}"
            raise BromicSerialError(message) from err
    ...
    async def async_send_command(
        self, id_location: int, button_code: int, retries: int = MAX_RETRIES
    ) -> BromicResponse:
        """
        ...
        if not self._connected:
            ...
            raise BromicConnectionError(message)
        ...
        for attempt in range(retries + 1):
            try:
                ...
                if attempt < retries:
                    ...
                    _LOGGER.exception("Command failed after %d attempts", retries + 1)
                    raise
            else:
                if response.success:
        ...
        raise BromicTimeoutError(message)
    ...
    def _send_command_sync(self, command: BromicCommand) -> BromicResponse:
        ...
        with self._lock:
            ...
            try:
                ...
                message = f"Command timeout: {err}"
                raise BromicTimeoutError(message) from err
            except serial.SerialException as err:
