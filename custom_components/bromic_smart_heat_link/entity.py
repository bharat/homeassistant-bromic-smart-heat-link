...
class BromicEntity(Entity):
    ...
    def __init__(
        ...
        self._attr_unique_id = f"{DOMAIN}_{port_id}_{id_location}_ch{channel}_{entity_type}"
    ...
    async def async_send_command(self, button_code: int) -> bool:
        ...
        """
        try:
            response = await self._hub.async_send_command(self._id_location, button_code)
            ...
            if response.success:
                ...
                _LOGGER.debug(
                    ...
                    self._id_location, button_code, self.entity_id
                )
                return True
            else:
                self._error_count += 1
                _LOGGER.warning(
                    ...
                    self._id_location, button_code, self.entity_id, response.message
                )
                return False
            ...
            _LOGGER.error(
                "Command exception: ID=%d, Button=%d, Entity=%s, Error=%s",
                self._id_location, button_code, self.entity_id, err
            )
            return False
